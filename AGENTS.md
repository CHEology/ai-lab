...
- Before creating or editing writing, citations, reading templates or reading styles, read [the writing design specification](docs/WRITING-DESIGN.md). It is the authority for typography, note placement, source descriptions, accessible reference/back links and verification. Keep its entry points in README, DESIGN and ADDING-WRITING in sync. This is repository guidance, not a work to list or publish on the reader-facing site.
...
- The approved Writing section is `/writing/`, with a `Blue Note 回应` folder; `writing/entries.json` records each entry's actual state. Keep each response's final URL segment identical to its source title, including Chinese, spaces and punctuation. Encode URL components, never replace Chinese with pinyin.
- Writing pages share the site's palette and directory vocabulary; reading text uses a restrained serif column. Use semantic Markdown footnotes for evidence, with visible endnotes and exact return links; do not append parenthesized domain names to paragraphs. The original work may be linked at its first substantive title mention and in the generated footer. Pending pages may show “回应尚未发布。” and “待写”, have no invented body or model attribution, and use `noindex, follow`.
...
- The final delivering agent must verify the content-creation model's byline before handing off or publishing completed AI writing (user instruction, 2026-09-09). Populate `byline` from the user's confirmation or reliable creation records; if the model is unknown, ask the user instead of silently omitting it. Credit the model that created the content, not an agent that only edited formatting or published it; never infer provenance from the delivering agent's identity. Display `Model name · YYYY-MM-DD`, e.g. `GPT-6 · 2026-09-05`, preserving the actual first publication date. Only omit a completed work's byline if the user explicitly requests it; pending pages remain unsigned. See [the writing design specification](docs/WRITING-DESIGN.md).
- The user has authorized direct publication of requested site and content updates (2026-09-05). After checks pass, commit, push to `main`, and verify deployment and the live page without asking for another publication confirmation. Clarifying unknown authorship is separate from publication approval. Follow any later explicit instruction to keep work local or in draft.
