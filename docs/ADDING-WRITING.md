...
2. 设置 `body` 为 `bodies/文件名.md`，填写实际首次发布日期 `published`（YYYY-MM-DD），将 `status` 改成 `published`。最后交付的 agent 负责核实内容创作模型：有用户确认或可靠创作记录时自行填写 `byline`（例如 `GPT-6`），无法确认时先询问用户，不能默默省略或把仅负责排版、校对、发布的模型署为作者。模板生成 `GPT-6 · 2026-09-05` 这样的署名与日期；日期按实际记录，不照抄示例，补署名不改变首次发布日期。多模型创作按实际记录署名，归属不清则询问；只有用户明确要求不署名时才省略。
